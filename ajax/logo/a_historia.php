...
#verificamos que exista el archivo imagefilee
if (isset($_FILES["imagefilee"])):
...
$nombre_image = time()."_".basename($_FILES["imagefilee"]["name"]);
...
$tamañoImagen=$_FILES["imagefilee"]["size"];
/* Inicio Validacion*/
...
if(($tipoDImagen != "jpg" && $tipoDImagen != "png" && $tipoDImagen != "jpeg" && $tipoDImagen != "gif" ) and $tamañoImagen>0) {
	...
	if ($tamañoImagen>0){
	#Muevo el archivo imagefilee a la url $cargo_file
		move_uploaded_file($_FILES["imagefilee"]["tmp_name"], $cargo_file);
...
<?php if ($queryExecute): ?>
	...
	<br><span class="btn badge-success ">"La imagen se actualizo exitosamente"...</span>
<?php endif; ?>
