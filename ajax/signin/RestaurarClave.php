...
include '../../config/fun_info.php';#info de la empresa
 ...
 $AInstitucion = dataEmpresa("abreviatura");  
    ...
    /*$FechaClave = date_default_timezone_set('America/El_Salvador');# Zona Horaria 
    $FechaClave = date("sm"); # segundo y mes*/
    $random_int = random_int(1, 3000); $segundoRun = date('s');/*Un numero aleatorio se suma a un segundo*/
    $FechaClave = $random_int+$segundoRun;
     $claveNueva = $FechaClave;

     /*encripto la clave*/   $clavehash=hash("SHA256",$claveNueva); 

// echo $claveNueva;
     
		#consulta sql a ejecutar
		$sql= $conexionPdo->prepare("UPDATE pm_usuario SET clave='$clavehash' WHERE  email='$email'");
		#ejecutando consulta...
		$query_SaveNuevaClave = $sql->execute();/*mysqli_query($con,$sql);*/

		if ($query_SaveNuevaClave){
...
if (isset($query_SaveNuevaClave)) {
	if ($query_SaveNuevaClave) {

  #consulta para imprimir el nombre del grado
   $queryNameUSUa4R = $conexionPdo->query("SELECT * FROM pm_usuario WHERE email='$email'")->fetchAll(PDO::  FETCH_OBJ);
   foreach ($queryNameUSUa4R  as $N){$NomB3u = $N->usuario; }		
...
$asunto = "Restauracion de contraseña $year   |    $name_web ";/*ENCABEZADO*/
...
<head>
  <meta charset='utf-8'>
  <title>Restauracion de Contraseña $year  | $name_web </title>
...
c0 6.62695 -5.37305 12 -12 12h-12v100c0 6.62695 -5.37305 12 -12 12h-64c-6.62695 0 -12 -5.37305 -12 -12v-24c0 -6.62695 5.37305 -12 12 -12h12v-64h-12c-6.62695 0 -12 -5.37305 -12 -12v-24c0 -6.62695 5.37305 -12 12 -12h88c6.62695 0 12 5.37305 12 12z'  fill='#000'/>
      ...
      <h1 class='text-dark'>Tu contraseña se restauro satisfactoriamente </h1>
      <h3 class=' text-muted'>$year.</h3>
        ...
        <div class='row'>
         ...
         <h2 class='text-dark'>TU USUARIO: $NomB3u </h2>
         <h2 class='text-dark'>TU CLAVE DE ACCESO: $claveNueva </h2>
         <h2>ENLACE PARA ACCEDER A TU EXPEDIENTE:</h2>
         <a  href='index.php' class=' my-2 btn btn-lg btn-info'>ir al Login</a>


        <h2>Tu contraseña se restauro correctamente !</h2>
...
Swal.fire({
                       icon:'error',
                       title:'Error el correo no existe en el sistema...',
                         html: '<h4><?php echo $error; ?></h4>',
                          grow: 'fullscreen',
                       footer: 'Rellena el campo correctamente. '
                   });
...
Swal.fire({
                       ...
                       html: '<h4><?php echo $message; ?></h4><br>recuerda tu usuario',
                       grow: 'fullscreen',
                       footer: 'Te hemos enviado un correo con tu nueva contraseña.'
                   });
